	...
	return new Stream.Transform({
		...
		transform(chunk, encoding, callback) {
			chunk.contents = Buffer.from(
				chunk.contents.toString(encoding).replaceAll(searchValue, repaceValue),
				encoding
			);
			callback(null, chunk);
		},
...
function newer(relatedTo) {
	return new Stream.Transform({
		...
		transform(chunk, encoding, callback) {
			Fs.stat(
				Path.join(relatedTo, Path.relative(chunk.base, chunk.path)),
				function (relatedError, relatedStat) {
					callback(
						null,
						relatedError || relatedStat.mtime < chunk.stat.mtime ? chunk : null
					);
				}
			);
		},
...
function printPaintedMessage(message, module) {
	let errs = [
		...message.matchAll(
			new RegExp(
				/(?:[A-Za-z]+:*\\[а-яА-Яa-zA-Z-_.\\/]+)|('[а-яА-Яa-zA-Z-_.\\/]+')/gm
			)
		),
	]
		.map(function (curr) {
...
function copyStatic() {
	return Gulp.src(
		["./src/assets/static/**/*", "!./src/assets/static/img-raw/**/*"],
		{
			allowEmpty: true,
	...
	return Gulp.src("./src/assets/static/img-raw/icon/**/*.svg", {
		...
		.pipe(
			new Stream.Transform({
				...
				writableObjectMode: true,
				transform(chunk, encoding, callback) {
					let name = Path.relative(chunk.base, chunk.path)
						.replaceAll(Path.sep, "__")
						.replace(/\.[^/.]+$/, "");
					let path = pathTransform
						.toPosix(Path.relative(cwd(), chunk.path))
						.replace("/img-raw/", "/img/");
...
function imagemin() {
	return Gulp.src("./src/assets/static/img-raw/**/*", {
		...
		.pipe(Imagemin([svgo(), mozjpeg(), Pngquant()]))
...
function ttfToWoff() {
	return Gulp.src("./src/assets/static/font/**/*.ttf").pipe(
		new Stream.Transform({
			writableObjectMode: true,
			readableObjectMode: true,
			transform(chunk, encoding, callback) {
...
function watch() {
	...
	Gulp.watch(
		"./src/assets/static/img-raw/icon/**/*.svg",
		{
...
export default Gulp.series(
	Gulp.parallel(argv.ram ? nothing : cleanBuild, imagemin, makeIconsSCSS),
	Gulp.parallel(copyStatic, css, js, html),
	argv.fwatch ? Gulp.parallel(watch, browserSyncInit) : nothing
);
